...
public abstract class APIResource extends KloudlessObject {
	...
	private static javax.net.ssl.HttpsURLConnection createPostConnection(
		...
		try {
			if (params.containsKey("file") && params.containsKey("metadata")) {
				...
				Map<?, ?>json = GSON.fromJson((String) params.get("metadata"), Map.class);
				...
				output = conn.getOutputStream();
				output.write((crlf + twoHyphens + boundary + crlf).getBytes(CHARSET));
				...
				output.write((((String) params.get("metadata")).getBytes(CHARSET)));
				output.write((crlf + twoHyphens + boundary + crlf).getBytes(CHARSET));
	...
	private static KloudlessResponse makeURLConnectionRequest(
		...
		try {
			...
			return new KloudlessResponse(rCode, rBody, headers);
	...
	protected static KloudlessResponse request(APIResource.RequestMethod method,
	...
	protected static KloudlessResponse _request(APIResource.RequestMethod method,
			String path, Map<String, Object> params,
			String apiKey) throws AuthenticationException,
			InvalidRequestException, APIConnectionException, APIException {
		if ((Kloudless.apiKey == null || Kloudless.apiKey.length() == 0)
			...
			throw new AuthenticationException(
					"No API key provided. (HINT: set your API key using 'Kloudless.apiKey = <API-KEY>'. "
